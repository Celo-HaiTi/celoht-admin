...
| Section | Contents |
|---|---|
| **KPI row** | Current Balance, Inflows (90d), Outflows (90d), USDm Reserve - each with a period-over-period delta |
...
| `currency` | `USD` \| `USDm` \| `CELO` | The currency the transaction actually settled in |
| `description` | `text` | Human-readable context |
| `tx_hash` | `text`, nullable | On-chain reference when the transaction is a USDm/CELO transfer; null for fiat |
